...
Root [decision 007](https://github.com/thoughtkhoral/thought-khoral/blob/main/.ai/specs/decisions/007-a2a-agent-gateway-foundation.md) adds an explicit human `agent.task.start` composer for the pinned local reference agent's `summarize-context` and `extract-action-items` skills. The UI renders only persisted `requested`, meaningful `progressed`, optional `awaiting_external_input`, and terminal task events. It shows source identifiers with results and presents a validated HTTPS handoff as a user-clicked link with instruction and expiry; it never opens an external UX, handles its sensitive form inputs, creates authoritative progress events, or invokes A2A directly. The earlier mention-triggered Action Items path remains distinct.
